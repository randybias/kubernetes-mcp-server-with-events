...
- [ ] 1.1 Update tool schema in `pkg/toolsets/core/events.go`
...
- [ ] 1.2 Update logger constant in `pkg/events/notification.go`
      - Rename `LoggerResourceFaults = "kubernetes/resource-faults"` to `LoggerFaults = "kubernetes/faults"`
- [ ] 1.3 Update manager validation in `pkg/events/manager.go`
...
- [ ] 2.2 Remove old fault watcher from `pkg/events/watcher.go`
...
- [ ] 2.3 Remove old fault mode routing from `pkg/events/manager.go`
...
- [ ] 2.4 Clean up unused log enrichment code in `pkg/events/logs.go`
...
- [ ] 3.2 Remove obsolete test files/functions
...
- [ ] 4.2 Update `PHASE_12_TESTING.md` in worktree
      - Change all references from mode="resource-faults" to mode="faults"
- [ ] 4.3 Update main `README.md` if it documents event subscriptions
      - Search for subscription mode documentation
      - Update to reflect only two modes: events and faults
- [ ] 4.4 **VERIFY:** Documentation is accurate and consistent

## Phase 5: Integration verification

Goal: Verify the renamed implementation works end-to-end.

- [ ] 5.1 Build server binary: `go build -o kubernetes-mcp-server ./cmd/kubernetes-mcp-server`
- [ ] 5.2 Start server with mcp-inspector: `npx @modelcontextprotocol/inspector ./kubernetes-mcp-server`
- [ ] 5.3 Verify tool schema shows mode="faults" (not "resource-faults")
- [ ] 5.4 Create subscription with mode="faults"
- [ ] 5.5 Trigger pod crash and verify notification with logger="kubernetes/faults"
- [ ] 5.6 **VERIFY:** End-to-end flow works with renamed mode
